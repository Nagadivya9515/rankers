...
export default function HeroCarousel({ slides }: HeroCarouselProps) {
  ...
  return (
    ...
    >
      {/*
        Media box — the only part that crossfades between slides, so it
        needs a fixed height per breakpoint. All banners are authored at
        16:9 (1919×1080) with their own headline/CTA baked in edge-to-edge,
        so mobile shows each one uncropped at that native ratio
        (object-contain, no text overlay — stacking more text on top of
        these is what caused the mobile overlap before). `sm`+ keeps that
        same 16:9 box and crops slightly wider to overlay the HTML caption,
        which reads fine at those sizes. (Previously `lg` used 21:9, which
        is shorter than the source and cropped off the banner's own bottom
        feature strip — kept at 16:9 here so desktop shows the full banner
        too.)
      */}
      <div className="relative aspect-video w-full">
        {slides.map((slide, index) => (
          ...
          >
            ...
            <div
              aria-hidden="true"
              className="absolute inset-0 hidden sm:block sm:bg-gradient-to-r sm:from-brand-950/85 sm:via-brand-950/40 sm:to-transparent"
            />
            <div className="container-page absolute inset-0 hidden items-center sm:flex">
        ...
        {slides.length > 1 && (
          <>
            ...
            <div
              ...
              className="absolute bottom-2 left-1/2 flex -translate-x-1/2 gap-2 sm:bottom-6"
            >
              {slides.map((slide, index) => (
                <button
                  key={slide.id}
      ...
      {/*
        Mobile-only caption strip — sits below the image (never on top of
        it), so it can never overlap the banner's own baked-in text. Just
        the essentials (no description) since the image already carries
        most of the message. Not part of the crossfade stack above: it
        reads straight off `slides[active]`, so it's a plain content swap
        that always matches its own natural height (no clipping risk).
      */}
      <div className="bg-brand-900 px-5 py-6 text-center sm:hidden">
        <p className="eyebrow text-gold-400">{slides[active]?.eyebrow}</p>
        <h2 className="mt-2 text-xl font-extrabold leading-tight text-white">
          {slides[active]?.title}
        </h2>
        {slides[active] && (
          <Link href={slides[active].ctaHref} className="btn-primary mt-4">
